...
export const CardImageGET = async (req) => {
    try{
        const card = await prisma.imagecard.findMany()
...
export const CardImageGetByID = async (req , {params}) => {
    try {
        const id = params.id;
        const card = await prisma.imagecard.findUnique({
            where: {
                id: parseInt(id, 10),
            }
        });
